...
(ert-deftest inspector-tests--inspector-inspect-defun-top-sexp-simple-list ()
  (with-temp-buffer
    (insert "(list 1 2 3)")
    (goto-char (point-min))
...
(ert-deftest inspector-tests--inspector-inspect-defun-top-sexp-simple-vector ()
  (with-temp-buffer
    (insert "(vector 1 2 3)")
    (goto-char (point-min))
    (inspector-inspect-defun)
    (let ((buffer-string (buffer-string)))
      ...
      (should (cl-search "3" buffer-string)))
    (inspector-quit)))
(ert-deftest inspector-tests--inspector-inspect-defun-simple-assoc ()
  (with-temp-buffer
    (insert "'( 'foo '((bar . 1) (foo . 2) (baz . 3)))")
    (goto-char (point-min))
    (inspector-inspect-defun)
    (let ((buffer-string (buffer-string)))
      (should (cl-search "proper list" buffer-string))
      (should (cl-search "foo" buffer-string))
      (should (cl-search "bar" buffer-string))
      (should (cl-search "1" buffer-string))
      (should (cl-search "foo" buffer-string))
      (should (cl-search "2" buffer-string))
      (should (cl-search "baz" buffer-string))
      (should (cl-search "3" buffer-string)))
    (inspector-quit)))


(ert-deftest inspector-tests--inspector-inspect-defun-simple-hash-table ()
  (with-temp-buffer
    ...
    (let ((buffer-string (buffer-string)))
      (should (cl-search "hash-table" buffer-string)))
    (inspector-quit)))
(ert-deftest inspector-tests--inspector-inspect-defun-complicated-list ()
  (with-temp-buffer
    (insert "(list 1 2 3 (list 4 5 6) 7 8 9)")
    (goto-char (point-min))
    (inspector-inspect-defun)
    (let ((buffer-string (buffer-string)))
      (should (cl-search "proper list" buffer-string))
      ...
      (should (cl-search "8" buffer-string))
      (should (cl-search "9" buffer-string)))
    (inspector-quit)))
(ert-deftest inspector-tests--inspector-inspect-defun-complicated-vector ()
  (with-temp-buffer
    (insert "(vector 1 2 3 (vector 4 5 6) 7 8 9)")
    (goto-char (point-min))
    (inspector-inspect-defun)
    (let ((buffer-string (buffer-string)))
      (should (cl-search "vector" buffer-string))
    ...
    (inspector-quit)))
(ert-deftest inspector-tests--inspector-inspect-defun-complicated-assoc( )
  (
   with-temp-buffer
   (insert "'('foo '((bar . 1) (foo . 2) (baz . 3) (quux . (list 4 5 6)) (quuux . 7) (quuuux . 8) (quuuuux . 9)))")
   (goto-char (point-min))
   (inspector-inspect-defun)
   (let ((buffer-string (buffer-string)))
     (should (cl-search "proper list" buffer-string))
     ...
     (should (cl-search "baz" buffer-string))
     (should (cl-search "3" buffer-string))
     (should (cl-search "quux" buffer-string))
     )
   (inspector-quit)))


(ert-deftest inspector-tests--inspector-inspect-region ()
  (with-temp-buffer
    (insert "(list 1 2 3)")
    (goto-char (point-min))
    (inspector-inspect-region (point) (point-max))
    (let ((buffer-string (buffer-string)))
      (should (cl-search "proper list" buffer-string))
      (should (cl-search "1" buffer-string))
      (should (cl-search "2" buffer-string))
